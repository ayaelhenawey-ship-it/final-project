...
export const updateMyProfile = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.body.password || req.body.email || req.body.phoneNumber) {
      return next(
        new AppError(
          "The password, email, or phone number cannot be updated from this path.",
          400,
        ),
      );
    }
    ...
    const allowedUpdates: any = {
      ...
      bio: req.body.bio,
      skills:
        normalizedSkills !== undefined ? normalizedSkills : req.body.skills,
...
// ==========================================
// 🔔 تحديث إعدادات الإشعارات الخاصة بالمستخدم
// ==========================================
export const updateNotificationSettings = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    // 1. استخراج الإعدادات اللي الفرونت إند بعتها من الـ Body
    const { mentions, chatMessages, jobAlerts, sounds } = req.body;

    // 2. تجهيز أمر التحديث (بندخل جوه الأوبجكت بتاع الإعدادات خطوة بخطوة)
    const updateQuery: any = {};

    if (mentions !== undefined)
      updateQuery["notificationSettings.mentions"] = mentions;
    if (chatMessages !== undefined)
      updateQuery["notificationSettings.chatMessages"] = chatMessages;
    if (jobAlerts !== undefined)
      updateQuery["notificationSettings.jobAlerts"] = jobAlerts;
    if (sounds !== undefined)
      updateQuery["notificationSettings.sounds"] = sounds;

    // 3. تحديث الداتا بيز وإرجاع النتيجة
    const updatedUser = await User.findByIdAndUpdate(
      req.user!._id,
      { $set: updateQuery },
      { new: true, runValidators: true },
